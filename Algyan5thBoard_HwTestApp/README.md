...
1. USBシリアルケーブルを接続する。
2. 電源(5V)を投入する。
	3.3V のUSBシリアルしか持っていない人は書き込み時にも電源を投入してください。	USBシリアルから 5V を供給している場合は不要です。
3. 書き込みモードにする。
	SW3 を押した状態で、SW2 を押して離すと書き込みモードになります。
4. Arduino IDE から書き込む。
...
*SW2 + SW3
	...
	書き込みができない（書き込みモードにならない）場合はこの二つのスイッチ周りに問題があると思います。
*WiFi機能
...
*明るさセンサー + サーボモーター
	...
	SW1 を押して LEDが点灯している間、シリアルにはセンサーの値を出力しています。	シリアルモニターを閉じてシリアルプロッターを開くとグラフが表示されます。
